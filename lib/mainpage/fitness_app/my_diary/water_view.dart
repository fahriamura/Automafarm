...
class WaterView extends StatefulWidget {
  const WaterView(
      {Key? key, this.mainScreenAnimationController, this.mainScreenAnimation, required this.UserID})
      : super(key: key);
  final int UserID;
...
class _WaterViewState extends State<WaterView> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: ActivityListData.fetchActivities(widget.UserID), // Ensure UserID is available in this scope
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          double getPercentage = (ActivityListData.getTotalLiter(ActivityListData.tabIconsList) / 4000) * 100;
                                        ...
                                        children: <Widget>[
                                          Padding(
                                            padding: const EdgeInsets.only(
                                                left: 4, bottom: 3),
                                            child: Text(
                                              '${ActivityListData.getTotalLiter(ActivityListData.tabIconsList)}',
